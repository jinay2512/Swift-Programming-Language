...
//-----------------------------------------------------------
{
   ...
   identifierTable.AddToTable(lexeme,FUNCTION_MODULE,datatype,lexeme);
   index = identifierTable.GetIndex(lexeme,isInTable);

// CODEGENERATION
   code.EnterModuleBody(FUNCTION_MODULE,index);
   code.ResetFrameData();
...
// Reserve frame-space for FUNCTION return value
   ...
   code.EmitUnformattedLine(line);
   code.EmitUnformattedLine("; **** =========");
   code.EmitFormattedLine(lexeme,"EQU","*");
// ENDCODEGENERATION
...
// CODEGENERATION
   ...
   code.EmitFormattedLine("","PUSH",operand);
   code.EmitFormattedLine("","JMP","FUNCTIONENDERROR");
   code.EmitUnformattedLine("; **** =========");
   sprintf(line,"; **** END (%4d)",tokens[0].sourceLineNumber);
   code.EmitUnformattedLine(line);
   code.EmitUnformattedLine("; **** =========");
...
// STATICSEMANTICS
   if ( !(code.IsInModuleBody(PROCEDURE_MODULE) || code.IsInModuleBody(FUNCTION_MODULE)) )
      ProcessCompilerError(tokens[0].sourceLineNumber,tokens[0].sourceLineIndex,
...
// ENDCODEGENERATION
   ...
   else//if ( code.IsInModuleBody(FUNCTION_MODULE) )
   {
