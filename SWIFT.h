//-----------------------------------------------------------
// Jinay Jani
// SWIFT common routines assumes standard set of includes and macro 
//    definitions precede the #include ".\SWIFT.h" of this file
// SWIFT.h
//-----------------------------------------------------------
...
enum IDENTIFIERTYPE
{
   GLOBAL_VARIABLE,      // static global data
   PROGRAM_VARIABLE,     // static local data

   MODULE_VARIABLE,      // automatic, frame-based data
   ...
   PROCEDURE_MODULE,      // module name
// UPDATESFOR SPL5.h
   FUNCTION_MODULE,     // module name
// ENDUPDATESFOR SPL5.h
};
...
public:
   //-----------------------------------------------------------
   SWIFTEXCEPTION(char description[])
   //-----------------------------------------------------------
   {
...
public:
   ...
   ~LISTER();
   void OpenFile(char sourceFileName[]);
   void ListSourceLine(int sourceLineNumber,char sourceLine[]);
...
//-----------------------------------------------------------
void LISTER::OpenFile(char sourceFileName[])
//-----------------------------------------------------------
{
   char fullFileName[80+1];

   strcpy(this->sourceFileName,sourceFileName);
   strcat(this->sourceFileName,".swift");
   strcpy(fullFileName,sourceFileName);
   strcat(fullFileName,".lst");
   LIST.open(fullFileName,ios::out);
   if ( !LIST ) throw( SWIFTEXCEPTION("Unable to open list file") );
   ListTopOfPageHeader();
}

//-----------------------------------------------------------
void LISTER::ListSourceLine(int sourceLineNumber,char sourceLine[])
//-----------------------------------------------------------
{
...
//-----------------------------------------------------------
void LISTER::ListInformationLine(char information[])
//-----------------------------------------------------------
{
   if ( linesOnPage >= LINESPERPAGE )
   {
...
private:
   ...
   bool atEOP;
   int numberCallbacks;
   void (*CallbackFunctions[CALLBACKSALLOWED+1])(int sourceLineNumber,char sourceLine[]);
...
public:
   ...
   void AddCallbackFunction(void (*CallbackFunction)(int sourceLineNumber,char sourceLine[]));
private:
   void ReadSourceLine();
};
...
//-----------------------------------------------------------
template<int CALLBACKSALLOWED>
void READER<CALLBACKSALLOWED>::OpenFile(char sourceFileName[])
//-----------------------------------------------------------
{
   char fullFileName[80+1];

   strcpy(fullFileName,sourceFileName);
   strcat(fullFileName,".swift");
   SOURCE.open(fullFileName,ios::in);
   if ( !SOURCE ) throw( SWIFTEXCEPTION("Unable to open source file") );
...
// index in [ 0,LOOKAHEAD ] where index = 0 means last GetNextCharacter() returned
   if ( (0 <= index) && (index <= LOOKAHEAD) )
      ...
      throw( SWIFTEXCEPTION("GetLookAheadCharacter() index out-of-range") );
}

//-----------------------------------------------------------
template<int CALLBACKSALLOWED>
void READER<CALLBACKSALLOWED>::AddCallbackFunction(void (*CallbackFunction)(int sourceLineNumber,char sourceLine[]))
//-----------------------------------------------------------
{
   if ( numberCallbacks <= CALLBACKSALLOWED )
      CallbackFunctions[++numberCallbacks] = CallbackFunction;
   else
      throw( SWIFTEXCEPTION("Too many callback functions") );
}
...
{
/*
   SWIFT static data--global variable definitions, string literals, and PROGRAM
      module variables--are parsed very early in the compile and near the end 
      of the compile, respectively. As a result, their STM code must be "stored"
      because it cannot be emitted until *AFTER* the entire SWIFT program has
      been parsed.
*/
private:
   struct DATARECORD
...
public:
   ...
   ~CODE();
   void OpenFile(char sourceFileName[]);
   void AddRWToStaticData(int operand,char comment[],char reference[]);
   void AddDWToStaticData(char operand[],char comment[],char reference[]);
   void AddDSToStaticData(char operand[],char comment[],char reference[]);
   void EmitStaticData();
   int LabelSuffix();
   void EmitFormattedLine(char label[],char mnemonic[],char operand[] = "",char comment[] = "");
   void EmitUnformattedLine(char line[]);

   void ResetFrameData();
   void IncrementFBOffset(int increment);
   int GetFBOffset();
   void AddInstructionToFrameData(char mnemonic[],char operand[],char comment[] = "");
   void EmitFrameData();
...
//--------------------------------------------------
void CODE::OpenFile(char sourceFileName[])
//--------------------------------------------------
{
   strcpy(codeFileName,sourceFileName);
   strcat(codeFileName,".stm");
   STM.open(codeFileName,ios::out);
   if ( !STM ) throw( SWIFTEXCEPTION("Unable to open code file") );
}

//--------------------------------------------------
void CODE::AddRWToStaticData(int operand,char comment[],char reference[])
//--------------------------------------------------
{
   DATARECORD r;
...
//--------------------------------------------------
void CODE::AddDWToStaticData(char operand[],char comment[],char reference[])
//--------------------------------------------------
{
   DATARECORD r;
...
//--------------------------------------------------
void CODE::AddDSToStaticData(char operand[],char comment[],char reference[])
//--------------------------------------------------
{
   DATARECORD r;
...
//--------------------------------------------------
void CODE::EmitFormattedLine(char label[],char mnemonic[],char operand[],char comment[])
//--------------------------------------------------
{
/*
         1         2         3         4         5         6         7         8
...
//--------------------------------------------------
void CODE::EmitUnformattedLine(char line[])
//--------------------------------------------------
{
   STM << line << endl;
}
...
//--------------------------------------------------
void CODE::AddInstructionToFrameData(char mnemonic[],char operand[],char comment[])
//--------------------------------------------------
{
   DATARECORD r;
...
private:
   static const char IDENTIFIERTYPENAMES[][16+1];
...
public:
   ...
   ~IDENTIFIERTABLE();
   int GetIndex(char lexeme[],bool &isInTable);
   void AddToTable(char lexeme[],IDENTIFIERTYPE identifierType,
                   DATATYPE datatype,char reference[]);
   void EnterNestedStaticScope();
   void ExitNestedStaticScope();
   void DisplayTableContents(char description[]);

// Assume index was determined by a prior successful call to GetIndex()
...
const char IDENTIFIERTABLE::IDENTIFIERTYPENAMES[][16+1] =
{
   "GLOBAL_VARIABLE",
   "PROGRAM_VARIABLE",
...
//-----------------------------------------------------------
int IDENTIFIERTABLE::GetIndex(char lexeme[],bool &isInTable)
//-----------------------------------------------------------
{
/*
   Try to find identifier's lexeme in identifier table working from the end of
      the table toward the beginning, but consider only those identifiers that are
      still in scope.
*/
   ...
   while ( (index >= 1) && !isInTable )
   {
      ...
      {
         isInTable = true;
         isInScope = identifierTable[index].scope == scopes;
      }
      else
         index -= 1;
   }
...
#ifdef TRACEIDENTIFIERTABLE
{
   ...
   if ( isInTable )
      sprintf(information,"Found identifier \"%s\" at index = %d (%s)",
         lexeme,index,((isInScope) ? "is in current scope" : "not in current scope"));
   else
...
//-----------------------------------------------------------
void IDENTIFIERTABLE::AddToTable(char lexeme[],IDENTIFIERTYPE identifierType,
                                 DATATYPE datatype,char reference[])
//-----------------------------------------------------------
{
/*
...
*/
   if ( identifiers > capacity )
      throw( SWIFTEXCEPTION("Identifier table capacity exceeded") );
   else
   {
      identifiers++;
...
#ifdef TRACEIDENTIFIERTABLE
{
   ...
   sprintf(information,"Added identifier \"%s\" at index = %d, reference = %s, identifier type = %s, data type = %s",
...
{
/*
   Remove the identifiers of *ALL* local variables in the module scope just 
      ended *EXCEPT* for subprogram module formal parameters.

      Mark the formal parameters so they will not be "found" by subsequent 
      searches. The formal parameters are retained so references to their 
      module can be compiled. *Note* The subprogram module identifier is 
      retained because it is in the scope just re-entered.
*/
   identifiers = scopeTable[scopes--];
 // UPDATESFOR SPL5.h
   if ( (identifierTable[identifiers].identifierType == PROCEDURE_MODULE) ||
        (identifierTable[identifiers].identifierType == FUNCTION_MODULE) )
// ENDUPDATESFOR SPL5.h
...
//--------------------------------------------------
void IDENTIFIERTABLE::DisplayTableContents(char description[])
//--------------------------------------------------
{
/*
================================================================================
description
  ...
  # scope Datatype  Type             Reference            Lexeme
--- ----- --------- ---------------- -------------------- --------------------
XXX   XXX XXXXXXXXX XXXXXXXXXXXXXXXX XXXXXXXXXXXXXXXXXXXX XX....
  .     . .         .                .                    .
  .     . .         .                .                    .
================================================================================
*/
   ...
   lister->ListInformationLine("  # Scope Datatype  Type             Reference            Lexeme");
   lister->ListInformationLine("--- ----- --------- ---------------- -------------------- --------------------");
   for (int i = 1; i <= identifiers; i++)
   {
      char information[SOURCELINELENGTH+1];

      sprintf(information,"%3d   %3d %-9s %-16s %-20.20s %s",
