//-----------------------------------------------------------
// Dr. Art Hanna
// Simple Target Machine (STM) for SPL2016 featuring a two-pass 
//    assembler and simulation of a stack-based ISA
// ***NOTE*** Need to test GETAE and ADRAE; COPYAA; and COPYS

#define VERSION "August 18, 2016"
...
<hit>             ::= <dit> | A | B | C | D | E | F
<character>       ::= '<ASCIICharacter>'        || *Note* escape \ and ' with \
<string>          ::= "{ <ASCIICharacter> }*"   || *Note* escape \ and " with \
<letter>          ::= A | B | ... | Z | a | b | ... | z
...
//-----------------------------------------------------------
{
   ...
   void ReadSourceLine();
   void DefineIdentifierInTable(char lexeme[],int value);
   bool IdentifierIsInTable(char lexeme[]);
   int FindIdentifierInTable(char lexeme[]);
   int ATOI16(char lexeme[]);
   WORD ATOF16(char lexeme[]);
...
//-----------------------------------------------------------
{
   ...
   void RecordSyntaxError(char syntaxError[]);
   int ATOI16(char lexeme[]);
...
// <string>             ::= " { <ASCIICharacter }* "
         case '"': 
            *token = STRING;
            i = 0;
            GetNextCharacter();
            while ( (nextCharacter != '"') && (nextCharacter != EOLC) )
            {
               if ( nextCharacter == '\\' )
                  GetNextCharacter();
               lexeme[i++] = nextCharacter;
            ...
            if ( nextCharacter == EOLC )
               RecordSyntaxError("String terminated by end-of-line");
            lexeme[i] = '\0';
            GetNextCharacter();
            break;
         case '\'':
...
//-----------------------------------------------------------
{
   bool IdentifierIsInTable(char lexeme[]);
   int FindIdentifierInTable(char lexeme[]);
   void ProcessRunTimeError(char error[],bool isFatalError);
